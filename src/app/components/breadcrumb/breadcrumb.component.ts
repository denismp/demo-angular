    ...
    template: `
        <ul [class.breadcrumb]="useBootstrap">
            <li *ngFor="let url of _urls; let last = last" [ngClass]="{'breadcrumb-item': useBootstrap, 'active': last}"> <!-- disable link of last item -->
                <a href="javascript:void(0);" *ngIf="!last && url == prefix" (click)="navigateTo('/')">{{ url }}</a>
                <a href="javascript:void(0);" *ngIf="!last && url != prefix" (click)="navigateTo(url)">{{ friendlyName(url) }}</a>
